...
namespace LCUSharp
{
    ...
    public class LeagueClientApi : ILeagueClientApi
    {
        ...
        public static async Task<LeagueClientApi> ConnectAsync(CancellationToken cancellationToken)
        {
            var (port, token) = await GetAuthCredentialsAsync(cancellationToken).ConfigureAwait(false);
            if (cancellationToken.IsCancellationRequested) throw new TaskCanceledException();
            var eventHandler = new LeagueEventHandler(port, token);
            var api = new LeagueClientApi(port, token, eventHandler);
            return await EnsureConnectionAsync(api,cancellationToken).ConfigureAwait(false);
        }
        ...
        public async Task ReconnectAsync(CancellationToken cancellationToken)
        {
            ...
            await EnsureConnectionAsync(this,cancellationToken).ConfigureAwait(false);
        }
        ...
        private static async Task<(int port, string token)> GetAuthCredentialsAsync(CancellationToken cancellationToken)
        {
            await _processHandler.WaitForProcessAsync(cancellationToken).ConfigureAwait(false);
            if (cancellationToken.IsCancellationRequested) throw new TaskCanceledException();
