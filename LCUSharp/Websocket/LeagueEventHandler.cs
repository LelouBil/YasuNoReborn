...
namespace LCUSharp.Websocket
{
    ...
    internal class LeagueEventHandler : ILeagueEventHandler
    {
        ...
        public void ChangeSettings(int port, string token)
        {
            ...
            _webSocket
                ...
                .Subscribe(msg =>
                {
                    ...
                    var toLaunchHandlers = new List<EventHandler<LeagueEvent>>(eventHandlers);
                    
                    toLaunchHandlers.ForEach(eventHandler => eventHandler?.Invoke(this, leagueEvent));
                });
        }
        ...
        {
            var unsubscribe = _subscribers.Remove(uri);
            return unsubscribe;
        }

        public bool Unsubscribe(string uri, EventHandler<LeagueEvent> eventHandler)
        {
            if (!_subscribers.ContainsKey(uri)) return false;
            var eventHandlers = _subscribers[uri];
            var unsubscribe = eventHandlers.Remove(eventHandler);
            return unsubscribe;
        }
